...
public final class CollectionUtils {
  ...
  private static <T extends Comparable<T>> void eqBrute(List<T> list, int lo, int hi) {
    if (hi - lo == 1) {
      ...
      int pmin;
      if (hi - lo == 2) {
        pmin = list.get(lo).compareTo(list.get(lo + 1)) < 0 ? lo : lo + 1;
        pmin = list.get(pmin).compareTo(list.get(lo + 2)) < 0 ? pmin : lo + 2;
        if (pmin != lo) {
          comparable = list.get(lo);
          list.set(lo, list.get(pmin));
          list.set(pmin, (T) comparable);
        }
  ...
  public static <T extends Comparable<T>> void insertionSort(List<Comparable> list) {

...
public final class DocumentSkill extends DocumentBase {
  ...
  protected void resetTable() {
    if (!this.usedTables.isEmpty()) {
      Iterator var1 = this.tables.keySet().iterator();

      while(var1.hasNext()) {
        String table = (String)var1.next();
        if (!this.usedTables.contains(table)) {
          _log.warn("Unused table " + table + " for skill " + this.currentSkill.id);
        }
      }
    }
  ...
  protected Object getTableValue(String name) {
    ...
    if (values == null) {
      _log.error("No table " + name + " for skill " + this.currentSkill.id);
      return 0;
    } else if (!values.containsKey(this.currentSkill.currentLevel)) {
      _log.error("No value in table " + name + " for skill " + this.currentSkill.id + " at level " + this.currentSkill.currentLevel);
      return 0;
    } else {
  ...
  protected Object getTableValue(String name, int level) {
    ...
    if (values == null) {
      ...
      _log.error("No value in table " + name + " for skill " + this.currentSkill.id + " at level " + level);
      return 0;
    } else {
      this.usedTables.add(name);
  ...
  private void loadTable(Node tableNode, int skillLevelOffset, int levels) {
    ...
    for(int tblContIdx = 0; tblContIdx < tableContent.length; ++tblContIdx) {
      ...
      if (((Map)globalTableLevels).containsKey(skillLvl)) {
        _log.error("Duplicate skill level " + skillLvl + " in table " + tableName + " in skill " + this.currentSkill.id);
        return;
      }
  ...
  protected void parseSkill(Node n) {
    ...
    try {
      ...
      for(int skillChildNodesLen = skillRootNode.getChildNodes().getLength(); skillChildNodeIdx < skillChildNodesLen; ++skillChildNodeIdx) {
        ...
        if (skillNodeName.startsWith("enchant")) {
          ...
          try {
            ...
            _log.error("Wrong enchant " + skillNodeName + " in skill " + skillId);
            break;
          }
      ...
      _log.error("Error loading skill " + skillId, var20);
    }

...
public class DoorTemplate extends CharTemplate {
  ...
  private void setAI(String ai) {
    ...
    if (classAI == null) {
      log.error("Not found ai class for ai: " + ai + ". DoorId: " + this._id);
    } else {
    ...
    if (this._classAI.isAnnotationPresent(Deprecated.class)) {
      log.error("Ai type: " + ai + ", is deprecated. DoorId: " + this._id);
    }
  ...
  public CharacterAI getNewAI(DoorInstance door) {
    try {
      ...
      log.error("Unable to create ai of doorId " + this._id, var3);

...
WHERE char_id NOT IN (SELECT obj_Id FROM characters);
DELETE
FROM character_hennas
WHERE char_obj_id NOT IN (SELECT obj_Id FROM characters);
DELETE
...
WHERE object_id NOT IN (SELECT obj_Id FROM characters);
DELETE
...
WHERE obj_id NOT IN (SELECT obj_Id FROM characters);
DELETE
FROM pets
WHERE item_obj_id NOT IN (SELECT item_id FROM items);
DELETE
FROM siege_clans
WHERE clan_id NOT IN (SELECT clan_id FROM clan_data);
DELETE
FROM items
WHERE owner_id NOT IN (SELECT obj_Id FROM characters)
  AND owner_id NOT IN (SELECT clan_id FROM clan_data)
  AND owner_id NOT IN (SELECT objId FROM pets);

UPDATE characters
SET clanid=0
WHERE clanid NOT IN (SELECT clan_id FROM clan_data);
UPDATE clan_data
SET ally_id=0
WHERE ally_id NOT IN (SELECT ally_id FROM ally_data);

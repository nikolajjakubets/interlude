...
DELETE
FROM ` items `
WHERE ` items `.` item_id ` = iItemID LIMIT 1;
DELETE
FROM ` items_duration `
WHERE ` items_duration `.` item_id ` = iItemID LIMIT 1;
DELETE
FROM ` items_period `
WHERE ` items_period `.` item_id ` = iItemID LIMIT 1;
DELETE
FROM ` items_attributes `
WHERE ` items_attributes `.` item_id ` = iItemID LIMIT 1;
DELETE
FROM ` items_variation `
WHERE ` items_variation `.` item_id ` = iItemID LIMIT 1;
DELETE
FROM ` items_options `
WHERE ` items_options `.` item_id ` = iItemID LIMIT 1;
END $$
...
IF
    ...
    OR
    (iDuration = 0)
    OR (iPeriod = 0) OR (iOwnerID = 0) THEN -- OR (eLoc = 'VOID')
    CALL `lip_DeleteItem`(iItemID);
LEAVE entry;
END IF;

INSERT LOW_PRIORITY INTO ` items ` (` item_id `,
                                    ` owner_id `,
                                    ` item_type `,
                                    ` amount `,
                                    ` location `,
...
INSERT LOW_PRIORITY INTO ` items_duration ` (` item_id `,
                                             ` duration `)
VALUES (iItemID,
        iDuration) ON DUPLICATE KEY
UPDATE
    ` duration ` = iDuration;
...
INSERT LOW_PRIORITY INTO ` items_period ` (` item_id `,
                                           ` period `)
VALUES (iItemID,
        iPeriod) ON DUPLICATE KEY
UPDATE
    ` period ` = iPeriod;
...
IF
    ((iAttackType > -2) AND (iAttackVal > 0))
    OR -- have attack or some defence in new item
    (iDefFire > 0)
    OR
    (iDefWater > 0)
    OR
...
INSERT LOW_PRIORITY INTO ` items_attributes ` (` item_id `,
                                               ` attack_type `,
                                               ` attack_value `,
                                               ` defence_fire `,
                                               ` defence_water `,
...
IF
    ...
    OR
    (iVariStat2 > 0)
    THEN -- have some variation(augumentation) in new item
INSERT LOW_PRIORITY INTO ` items_variation ` (` item_id `,
                                              ` stat1 `,
                                              ` stat2 `)
VALUES (iItemID,
        iVariStat1,
        iVariStat2) ON DUPLICATE KEY
UPDATE
    ` stat1 ` = iVariStat1,
    ` stat2 ` = iVariStat2;
...
IF
    ...
    OR
    (iDamaged > 0)
    OR (iItemEnergy > 0) OR (iCustomFlags > 0) OR (iItemVisType > 0) THEN -- have some rare or custom flag in item
INSERT LOW_PRIORITY INTO ` items_options ` (` item_id `,
                                            ` blessed `,
                                            ` damaged `,
                                            ` energy `,
                                            ` flags `,
...
WHEN
    ...
    OR
    (iRowCount = 2)
    OR (iRowCount = 3) THEN -- not changed or updated or Bug#46675(or toad for mysql future?)
BEGIN
IF iDuration > 0 THEN
UPDATE LOW_PRIORITY ` items_duration `
SET ` duration ` = iDuration
    WHERE ` item_id ` = iItemID LIMIT 1;
...
IF iPeriod > 0 THEN
UPDATE LOW_PRIORITY ` items_period `
SET ` period ` = iPeriod
    WHERE ` item_id ` = iItemID LIMIT 1;
...
IF
    ((iAttackType > -2) AND (iAttackVal > 0))
    OR -- have attack or some defence
    (iDefFire > 0)
    OR
    (iDefWater > 0)
    OR
...
IF
    ...
    OR
    (iVariStat2 > 0)
    THEN -- have some new variation(augumentation) in existing item
INSERT INTO ` items_variation ` (` item_id `,
                                 ` stat1 `,
                                 ` stat2 `)
VALUES (iItemID,
        iVariStat1,
...
IF
    ...
    OR
    (iDamaged > 0)
    OR (iItemEnergy > 0) OR (iCustomFlags > 0) OR (iItemVisType > 0) THEN -- have some new rare or custom flag in item
INSERT LOW_PRIORITY INTO ` items_options ` (` item_id `,
                                            ` blessed `,
                                            ` damaged `,
                                            ` energy `,
